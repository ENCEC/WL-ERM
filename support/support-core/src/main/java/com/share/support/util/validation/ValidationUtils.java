...
public abstract class ValidationUtils {
	...
	/**
	 * 必填字段验证
	 * @param fieldName 待验证字段名
	 * @param errorMsg  验证错误后的提示语
	 * @param errors 存储错误信息的对象
	 * @return 返回是否通过验证
	 */
	 ...
	 * 字符串长度必须在一定区间范围内验证
	 * @param fieldName
	 * @param interval 长度限定区间，如果传空的数组也不会报错，但验证将总是会通过
	 * @param errorMsg
	 * @param errors
	 * @return
	 */
	 ...
	 * @param fieldName
	 * @param obj
	 * @param customFunction 自定义验证方法名称，注意该方法必须在 obj 里用 public 声明，且返回值为 boolean 型，否则会抛出异常
	 * @param errorMsg
	 * @param errors
	 * @return
	 */
		...
		for (Method md : mds) {
			if (md.getName().equals(customFunction)) {
				...
				try {
					...
					CustomBean ret = (CustomBean) md.invoke(obj);
					if (!ret.getIsSuccess()) {
						errorMsg = StringUtils.isNotBlank(ret.getErrorMessage())?ret.getErrorMessage():errorMsg;
						errors.add(fieldName, errorMsg);
						return false;
					}
					Method setter = mirror.getSetter(ret.getSetValue(),ret.getSetType());
					if(!Objects.isNull(ret.getReturnResult())){
						setter.invoke(obj,ret.getReturnResult());
					}
					return true;
				} catch (Exception e) {
					errors.add(fieldName, errorMsg);
